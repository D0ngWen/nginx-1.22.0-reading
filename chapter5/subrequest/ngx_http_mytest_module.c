...
//myctx->stock数组中
        ...
        for (; pRecvBuf->pos != pRecvBuf->last; pRecvBuf->pos++)
        {
            if (*pRecvBuf->pos == ',')
            {
                ...
                flag++;
                myctx->stock[flag].data = pRecvBuf->pos + 1;
            }
...
//stock[…],Today current price: …, volumn: …
    ...
    int bodylen = output_format.len + myctx->stock[0].len
                  + myctx->stock[2].len + myctx->stock[4].len - 6;
...
//时传入的data参数就是myctx
    ...
    //子请求的URI前缀是/list，这是因为访问服务器的请求必须是类
//似/list?name=s_sh000001这样的URI，这与5.6.1节在nginx.conf中
//配置的子请求location中的URI是一致的
    ngx_str_t sub_location = ngx_string("/list");
    ngx_str_t sub_prefix = ngx_string("name=");
    ngx_str_t sub_arg;
    sub_arg.len = sub_prefix.len + r->args.len;
    sub_arg.data = ngx_palloc(r->pool, sub_arg.len);
    ngx_snprintf(sub_arg.data, sub_arg.len, "%V%V", &sub_prefix, &r->args);
...
//游服务器的响应全部保存在子请求的sr->upstream->buffer内存缓冲区中
    ngx_int_t rc = ngx_http_subrequest(r, &sub_location, &sub_arg, &sr, psr, 
